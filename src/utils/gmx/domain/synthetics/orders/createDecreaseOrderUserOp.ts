...
export type createDecreaseOrderUserOpProps = {
  account: string;
...
export type DecreaseOrderUserOpCallbacks = {
...
function getPendingOrderFromParams(
  chainId: number,
  p: createDecreaseOrderUserOpProps
) {
...
function createEncodedPayload({
  ...
  ps: createDecreaseOrderUserOpProps[];
  subaccount: Subaccount;
