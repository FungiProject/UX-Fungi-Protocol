...
import { useWebsocketProvider } from "../WebsocketContext/WebsocketContextProvider";
...
export function SyntheticsEventsProvider({
  ...
  eventLogHandlers.current = {
    ...
    OrderCancelled: (eventData: EventLogData, txnParams: EventTxnParams) => {
      ...
      if (orderStatuses[key]) {
        setOrderStatuses((old) =>
          updateByKey(old, key, { cancelledTxnHash: txnParams.transactionHash })
        );
      }
  ...
  useEffect(
    function subscribe() {
      ...
      const addressHash = ethers.utils.defaultAbiCoder.encode(
        ["address"],
        [currentAccount]
      );
      ...
      const EVENT_LOG2_TOPIC =
        eventEmitter.interface.getEventTopic("EventLog2");
      ...
      function handleEventLog(
        ...
        eventLogHandlers.current[eventName]?.(
          parseEventLogData(eventData),
          txnOpts
        );
      }
      ...
      const filters = [
        ...
        },
        {
          ...
          topics: [
            EVENT_LOG1_TOPIC,
            [ORDER_CANCELLED_HASH, ORDER_EXECUTED_HASH],
          ],
        },
        // NEW CONTRACTS
        {
