...
function getMarksWithLabel(marks: number[]) {
  return marks.reduce((marks, value, index) => {
    marks[index * 10] = `${value}%`;
    return marks;
  }, {} as { [key: number]: string });
}

export default function RebalanceSlider(p: Props) {
  ...
  const customHandle = useMemo(() => {
    return (props: any) => (
      <LeverageSliderHandle {...props} keyValueMap={keyValueMap} />
    );
  }, [keyValueMap]);
...
function LeverageSliderHandle({
  ...
  return (
    <SliderTooltip
      ...
      overlay={`${parseFloat(displayValue.toString()).toFixed(2)}%`}
      visible={dragging}
      placement="top"
      key={index}
    >
...
function generateEquallySpacedArray(
  min: number,
  max: number,
  shouldIncludeMax?: boolean
): number[] {
...
function generateKeyValueMap(marks: number[]) {
  ...
  const keyValueMap = Object.fromEntries(
    values.map((value, index) => [index, value])
  );
  const valueKeyMap = Object.fromEntries(
