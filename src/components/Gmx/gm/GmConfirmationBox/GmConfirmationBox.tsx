...
export function GmConfirmationBox({
  ...
  const longSymbol = market?.isSameCollaterals
    ? `${longToken?.symbol} Long`
    : longToken?.symbol;
  const shortSymbol = market?.isSameCollaterals
    ? `${shortToken?.symbol} Short`
    : shortToken?.symbol;
            ...
            <div>
              {isDeposit && (
                <div className="border-b-1 pb-4">
                  {[longTokenText, shortTokenText]
                    .filter(Boolean)
                    .map((text) => (
                      <div key={text} className="flex text-lg justify-center">
                        <span className="mr-2">Pay {text}</span>
                      </div>
                    ))}
                  ...
                  <div className="flex text-lg justify-center">
                    <span className="mr-2">Receive</span> {marketTokenText}
                  </div>
                </div>
              )}
              {!isDeposit && (
                <div className="border-b-1 pb-4">
                  <div className="flex text-lg justify-center">
                    <span className="mr-2">Pay</span>&nbsp;{marketTokenText}
                  </div>
                  <div className="flex items-center justify-center ">
                    <ArrowDownIcon className="h-7 w-7 my-1" />
                  </div>
                  {[longTokenText, shortTokenText]
                    ...
                    .map((text) => (
                      <div key={text} className="flex text-lg justify-center">
                        <span className="mr-2">Receive</span>&nbsp;{text}
                      </div>
                    ))}
                </div>
              )}
