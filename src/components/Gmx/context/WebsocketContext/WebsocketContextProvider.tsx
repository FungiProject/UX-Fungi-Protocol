  ...
  useEffect(
    function updateProviderEffect() {
      ...
      if (newProvider) {
        ...
        console.log(
          `ws provider for chain ${chainId} initialized at ${initializedTime.current}`
        );
      }
      ...
      return function cleanup() {
        ...
        // eslint-disable-next-line no-console
        console.log(
          `ws provider for chain ${chainId} disconnected at ${Date.now()}`
        );
      };
    },
      ...
      function nextHealthCheck() {
        ...
        if (isDevelopment() && isReconnectingIntervalPassed) {
          ...
          console.log(
            ...
            }, subs: ${Object.keys(wsProvider._subs).length}`
          );
        }

        if (
          isProviderInClosedState(wsProvider) &&
          isReconnectingIntervalPassed
        ) {
          ...
          // eslint-disable-next-line no-console
          console.log(
            "ws provider health check failed, reconnecting",
            initializedTime.current
          );
        } else {
          healthCheckTimerId.current = setTimeout(
            nextHealthCheck,
