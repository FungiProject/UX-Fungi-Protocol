...
export function useReferralsData(account?: string | null) {
  ...
  useEffect(() => {
    ...
    async function getChainReferralData(
      chainId: number
    ): Promise<ReferralsStats | undefined> {
      ...
      return getReferralsGraphClient(chainId)
        ...
        .then((res) => {
          ...
          res.data.distributions.forEach((d) => {
            const item = {
              ...
              markets: d.markets.map((market) =>
                ethers.utils.getAddress(market)
              ),
            ...
            if (
              d.typeId === RebateDistributionType.Rebate ||
              d.typeId === RebateDistributionType.Claim
            ) {
          ...
          function getAffiliateRebateUsd(data: {
            totalRebateUsd: BigNumberish;
            discountUsd: BigNumberish;
          }) {
            return bigNumberify(data.totalRebateUsd)!.sub(data.discountUsd);
          }

          const affiliateReferralCodesStats: ReferralCodeStats[] =
            res.data.affiliateStats.map((e) => ({
          ...
          const affiliateTotalStats: AffiliateTotalStats =
            res.data.affiliateStats.reduce(
              (acc: AffiliateTotalStats, cv) => {
                acc.trades = acc.trades + parseInt(cv.trades);
                acc.tradedReferralsCount =
                  acc.tradedReferralsCount + parseInt(cv.tradedReferralsCount);
                acc.registeredReferralsCount =
                  acc.registeredReferralsCount +
                  parseInt(cv.registeredReferralsCount);
                ...
                acc.affiliateRebateUsd = acc.affiliateRebateUsd.add(
                  getAffiliateRebateUsd(cv)
                );

                acc.v1Data.volume = acc.v1Data.volume.add(cv.v1Data.volume);
                acc.v1Data.totalRebateUsd = acc.v1Data.totalRebateUsd.add(
                  cv.v1Data.totalRebateUsd
                );
                acc.v1Data.discountUsd = acc.v1Data.discountUsd.add(
                  cv.v1Data.discountUsd
                );
                acc.v1Data.affiliateRebateUsd =
                  acc.v1Data.affiliateRebateUsd.add(
                    getAffiliateRebateUsd(cv.v1Data)
                  );

                acc.v2Data.volume = acc.v2Data.volume.add(cv.v2Data.volume);
                acc.v2Data.totalRebateUsd = acc.v2Data.totalRebateUsd.add(
                  cv.v2Data.totalRebateUsd
                );
                acc.v2Data.discountUsd = acc.v2Data.discountUsd.add(
                  cv.v2Data.discountUsd
                );
                acc.v2Data.affiliateRebateUsd =
                  acc.v2Data.affiliateRebateUsd.add(
                    getAffiliateRebateUsd(cv.v2Data)
                  );
                return acc;
              },
              {
          ...
          const traderReferralTotalStats: TraderReferralTotalStats = res.data
            .referralTotalStats
            ? {
                volume: bigNumberify(res.data.referralTotalStats.volume)!,
                discountUsd: bigNumberify(
                  res.data.referralTotalStats.discountUsd
                )!,
                v1Data: {
                  volume: bigNumberify(
                    res.data.referralTotalStats.v1Data.volume
                  )!,
                  discountUsd: bigNumberify(
                    res.data.referralTotalStats.v1Data.discountUsd
                  )!,
                },
                v2Data: {
                  volume: bigNumberify(
                    res.data.referralTotalStats.v2Data.volume
                  )!,
                  discountUsd: bigNumberify(
                    res.data.referralTotalStats.v2Data.discountUsd
                  )!,
                },
              }
            : {
          ...
          return {
            ...
            affiliateTotalStats,
            traderReferralTotalStats,
            codes: res.data.referralCodes.map((e) =>
              decodeReferralCode(e.code)
            ),
          } as ReferralsStats;
        });
    }
      ...
      .then((res) =>
        res.reduce(
          (accumulator, currentValue) => {
            ...
            const { affiliateTotalStats, traderReferralTotalStats } =
              currentValue;

            accumulator.total.registeredReferralsCount +=
              affiliateTotalStats.registeredReferralsCount;
            accumulator.total.affiliateVolume =
              accumulator.total.affiliateVolume.add(affiliateTotalStats.volume);
            accumulator.total.affiliateRebateUsd =
              accumulator.total.affiliateRebateUsd.add(
                affiliateTotalStats.affiliateRebateUsd
              );

            accumulator.total.discountUsd = accumulator.total.discountUsd.add(
              traderReferralTotalStats.discountUsd
            );
            accumulator.total.traderVolume = accumulator.total.traderVolume.add(
              traderReferralTotalStats.volume
            );
