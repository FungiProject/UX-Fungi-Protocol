...
import { getSwapPathOutputAddresses } from "../../../utils/gmx/domain/synthetics/trade";
import { useChainId } from "../../../utils/gmx/lib/chains";
import { formatTokenAmount, formatUsd } from "../../../utils/gmx/lib/numbers";
...
export function OrderStatusNotification({
  ...
  const pendingOrderKey = useMemo(
    () => getPendingOrderKey(pendingOrderData),
    [pendingOrderData]
  );
  ...
  const orderData = useMemo(() => {
    ...
    const marketInfo = getByKey(
      marketsInfoData,
      pendingOrderData.marketAddress
    );
    const initialCollateralToken = getByKey(
      tokensData,
      pendingOrderData.initialCollateralTokenAddress
    );
