...
import {
  ...
  encodeReferralCode,
} from "../../../utils/gmx/domain/referrals";
import { ARBITRUM, AVALANCHE } from "../../../utils/gmx/config/chains";
import {
  bigNumberify,
  formatAmount,
  removeTrailingZeros,
...
import { getRootUrl } from "../../../utils/gmx/lib/url";
...
export async function getReferralCodeTakenStatus(
  ...
  const takenOnArb =
    !isAddressZero(ownerArbitrum) &&
    (ownerArbitrum !== account ||
      (ownerArbitrum === account && chainId === ARBITRUM));
  const takenOnAvax =
    !isAddressZero(ownerAvax) &&
    (ownerAvax !== account || (ownerAvax === account && chainId === AVALANCHE));
  ...
  if (
    chainId === AVALANCHE
      ? referralCodeTakenInfo[ARBITRUM]
      : referralCodeTakenInfo[AVALANCHE]
  ) {
...
export function getSharePercentage(
  ...
  if (!discountShare || discountShare?.eq(0))
  ...
  return removeTrailingZeros(
    formatAmount(discountPercentage, decimals, 3, true)
  );
}
...
export function getTwitterShareUrl(referralCode) {
  const message = [
    "Trying out trading on @GMX_IO, up to 50x leverage on $BTC, $ETH 📈",
