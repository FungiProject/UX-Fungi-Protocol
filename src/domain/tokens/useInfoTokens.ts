 ...
 */
export async function getAllTokensWithBalances(alchemyClient: AlchemyMultichainClient, chainId: number, address: string){
    const balances = await getTokenBalancesAlchemy(alchemyClient,chainId,address);
    if(!balances) return
...
export const getLifiTokensWithBalance = async (chainId: number, tokensBalance: TokenBalance[]): Promise<TokenInfo[]> => {
    ...
    for (const token of await getLifiTokens(chainId)) {
        const matchingBalance = tokensBalance.find(tb => tb.contractAddress.toLowerCase() === token.address.toLowerCase());
        token.balance = matchingBalance ? BigNumber.from(matchingBalance.tokenBalance) : BigNumber.from(0);
    ...
    tokensWithBalance.sort((a, b) => {
        const aBalance = a.balance ? parseInt(ethers.utils.formatUnits(a.balance, a.decimals)) : 0;
        const bBalance = b.balance ? parseInt(ethers.utils.formatUnits(b.balance, b.decimals)) : 0;
